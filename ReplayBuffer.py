...
class CNNReplayBuffer(object):
    def __init__(self, capacity, frame_len):
        """Replay memory class
        Args:
            capacity (int): Max size of this memory
        """
        ...
        self.most_recent_states = []
        
    def fixlen(self, q_input):
        recent_states = copy.deepcopy(q_input)
        i = 0
        while len(recent_states) < self.frame_len:
            recent_states.append(recent_states[i])
            i += 1
        return recent_states    
            
    def get_q_input(self, state):
        recent_states = self.most_recent_states
        if len(recent_states) == self.frame_len:
            recent_states.pop(0)
            recent_states.append(state)
        else:
            recent_states.append(state)
            recent_states = self.fixlen(recent_states)
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        if len(self.most_recent_states) < self.frame_len:
            self.most_recent_states.append(state)
        else:
            self.most_recent_states.pop(0)
            self.most_recent_states.append(state)
        self.frame_idx += 1    
