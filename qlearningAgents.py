...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        self.Q = util.Counter() # A Counter is a dict with default 0
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.Q[state][action] = (1 - self.alpha) * self.Q[state][action] + self.alpha * (reward + self.discount * self.getValue(nextState))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        if len(actions) == 0: 
            ...
            best_Q_next= max([self.getQValue(nextState, action) for action in actions])
